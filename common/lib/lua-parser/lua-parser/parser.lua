...
local function throw(label)
  ...
  for i, labelinfo in ipairs(labels) do
    if labelinfo[1] == label then return T(i) end
  end

  error("Label not found: " .. label)
end

local function expect(patt, label) return patt + throw(label) end

-- regular combinators and auxiliary functions

local function token(patt) return patt * V("Skip") end

local function sym(str) return token(P(str)) end

local function kw(str) return token(P(str) * -V("IdRest")) end

local function dec(n) return n - 1 end

local function tagC(tag, patt) return Ct(Cg(Cp(), "pos") * Cg(Cc(tag), "tag") * patt * Cg(Cp() / dec, "end_pos")) end

local function unaryOp(op, e) return { tag = "Op", pos = e.pos, end_pos = e.end_pos, [1] = op, [2] = e } end
...
local function cut(s, idx, match) return idx, match end

local function chainOp(patt, sep, label) return Cmt(Cf(sepBy(patt, sep, label), binaryOp), cut) end

local function commaSep(patt, label) return sepBy(patt, sym(","), label) end
...
local G = {
  ...
  Space = space ^ 1,
  Comment = P("--") * V("LongStr") / function() return end + P("--") * (P(1) - P("\n")) ^ 0,
...
local validator = require("common.lib.lua-parser.lua-parser.validator")

...
local scope = require("common.lib.lua-parser.lua-parser.scope")
...
local function set_vararg(env, is_vararg) env["function"][env.fscope].is_vararg = is_vararg end
...
function traverse_parlist(env, parlist)
  ...
  if len > 0 and parlist[len].tag == "Dots" then is_vararg = true end

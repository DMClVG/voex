...
function snowball.ondraw(g)
  ...
  for _, e in pairs(g.world:query("snowball")) do
    if e ~= g.gamescreen.player then
      ...
      snowball_model:setRotation(
        0,
        -math.atan2(dz, math.sqrt(dx * dx + dy * dy)),
        math.atan2(e.y - camera[2], e.x - camera[1])
      )
...
function snowball.onupdate(g, dt)
  for _, e in pairs(g.world:query("snowball")) do
    ...
    if g.master == nil and physics.intersect_point_world(g.world, e.x, e.y, e.z) then g.world:tag(e, "destroyed") end
  end
end

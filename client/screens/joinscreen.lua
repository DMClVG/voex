...
function joinscreen.cleanup(g)
	for _, signal in ipairs(g.joinscreen.signals) do signal:destroy() end
	g.joinscreen = nil
end
...
function joinscreen.socket_ondisconnect(g)
	joinscreen.error(g,"connection closed")
end

function joinscreen.update(g)
	g.socket:service()
end

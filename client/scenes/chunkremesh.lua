                ...
                if id ~= 0 then
                    ...
                    if type(tile.tex) == "table" then
                        ...
                        if gettile(blockdatapointer, x - 1, y, z) == 0 then
                            addface(x, y, z, 0, 1, 2, (left % 16) / 16,
                                math.floor(left / 16) / 16, c2)
                        end
                        if gettile(blockdatapointer, x + 1, y, z) == 0 then
                            addface(x + 1, y, z, 0, 1, 2,
                                (right % 16) / 16, math.floor(right / 16) / 16, c2, true)
                        end
                        if gettile(blockdatapointer, x, y - 1, z) == 0 then
                            addface(x, y, z, 1, 0, 2, (front % 16) / 16,
                                math.floor(front / 16) / 16, c1, true)
                        end
                        if gettile(blockdatapointer, x, y + 1, z) == 0 then
                            addface(x, y + 1, z, 1, 0, 2,
                                (back % 16) / 16, math.floor(back / 16) / 16, c1)
                        end
                        if gettile(blockdatapointer, x, y, z - 1) == 0 then
                            addface(x, y, z, 1, 2, 0, (bottom % 16) / 16,
                                math.floor(bottom / 16) / 16, c3)
                        end
                        if gettile(blockdatapointer, x, y, z + 1) == 0 then
                            addface(x, y, z + 1, 1, 2, 0,
                                (top % 16) / 16, math.floor(top / 16) / 16, c1, true)
                        end
                    else
                        ...
                        if gettile(blockdatapointer, x, y, z + 1) == 0 then
                            addface(x, y, z + 1, 1, 2, 0, u1, v1, c1,
                                true)
                        end
                    end
                end

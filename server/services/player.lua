...
function player.view_onchunkinserted(g, p, c) p.master:send(packets.chunkadd(c:dump(), c.x, c.y, c.z)) end

function player.view_onchunkremoved(g, p, c) p.master:send(packets.chunkremove(c.x, c.y, c.z)) end
...
function player.view_onentityremoved(g, p, e) p.master:send(packets.entityremove(e.id)) end
...
function player.update(g, dt)
  ...
  for _, e in pairs(world.entities) do
    if e.remote then
      e.remote.x, e.remote.y, e.remote.z = e.x, e.y, e.z
    end
  end
end
